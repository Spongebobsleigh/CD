    ...
    while response is None:
        try:
            response = client.chat.completions.create(
                # model= 'gpt-4.1-nano', #'gpt-4-0613',#
                model= 'gpt-4o-mini', #'gpt-4-0613',#
                messages=[
    ...
    def _evaluate(self, model, result_file_manager):
        ...
        for row_idx, data in enumerate(tqdm(dataset['test'], desc="MMHal-Bench")):
            ...
            if self.result_file_manager.is_absent_sample(row_idx) == False:
                ...
                gpt_grade = -1
                scores_found = []
                # より柔軟なパターンマッチング: "Rating: 6" または "rating: 6" を探す
                response_lower = gpt4_response.lower()
                # パターン1: "rating: X" または "Rating: X" を探す
                for s in range(7):
                    if f'rating: {s}' in response_lower:
                        scores_found.append(s)
                # パターン2: 正規表現でより柔軟に探す
                if len(scores_found) == 0:
                    # "Rating: 6" や "rating: 6" の後に続く数字を探す
                    rating_pattern = re.search(r'rating\*{0,2}\s*:\s*(\d)', response_lower)
                    if rating_pattern:
                        score = int(rating_pattern.group(1))
                        if 0 <= score <= 6:
                ...
                if len(scores_found) == 1:
                    ...
                    print(f'Row {row_idx}, Response: {response[:30]}...')
                    print(f'GPT-4 Response: {response_lower[:]}...')
                    gpt_grade = 0
