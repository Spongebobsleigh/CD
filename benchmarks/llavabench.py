...
utc_plus_8 = pytz.timezone("Asia/Tokyo")  # You can also use 'Asia/Shanghai', 'Asia/Taipei', etc.
...
def get_eval(content: str, max_tokens: int):
    ...
    # gpt-5-nanoモデルはmax_completion_tokensを使用、それ以外はmax_tokensを使用
    payload = {
        "model": model,
        "messages": messages,
    }
    ...
    # モデル名に応じて適切なパラメータを設定
    if "gpt-5-nano" in model or "gpt-5-mini" in model:
        payload["max_completion_tokens"] = 4096
        # gpt-5-nanoはtemperature=0.0をサポートしていないため、設定しない（デフォルト値1が使用される）
    else:
    ...
    while True:
        try:
            ...
            if 'choices' not in response_data:
                print(f"エラー: レスポンスに'choices'キーがありません。レスポンス: {response_data}")
                time.sleep(NUM_SECONDS_TO_SLEEP)
                continue
                
            if not response_data['choices'] or len(response_data['choices']) == 0:
                print(f"エラー: 'choices'配列が空です。レスポンス: {response_data}")
                time.sleep(NUM_SECONDS_TO_SLEEP)
                continue
                
            if 'message' not in response_data['choices'][0]:
                print(f"エラー: 'message'キーがありません。レスポンス: {response_data}")
                time.sleep(NUM_SECONDS_TO_SLEEP)
                continue
                
            outputs = response_data['choices'][0]['message'].get('content')
            if outputs is None:
                print(f"エラー: 'content'がNoneです。レスポンス: {response_data}")
                time.sleep(NUM_SECONDS_TO_SLEEP)
                continue
            ...
            print(f"キーエラー: {e}")
            print(f"レスポンス: {response_data if 'response_data' in locals() else response.json() if 'response' in locals() else 'N/A'}")
            time.sleep(NUM_SECONDS_TO_SLEEP)
            ...
            if 'response' in locals():
                try:
                    ...
                    print(f"レスポンスJSON: {response.json()}")
                except:
                    pass
            time.sleep(NUM_SECONDS_TO_SLEEP)
